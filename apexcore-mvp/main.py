...
def heuristic_score(html: str) -> float:
    ...
    for marker in ["gpt", "chatgpt", "openai", "ki-generiert", "ai-generated", "powered by ai", "written by ai", "created with ai"]:
        if marker in html_lower:
            score += 15
    tw_count = sum(html_lower.count(w) for w in ["however", "moreover", "therefore", "furthermore", "additionally", "nevertheless", "consequently", "subsequently"])
    if len(html.split()) > 0 and tw_count / max(len(html.split()), 1) > 0.01:
        score += 25
    if not any(kw in html_lower for kw in ["ki-generiert", "ai-generated", "künstliche intelligenz", "erstellt mit ki", "generated by ai"]):
        score += 20
...
def assemble_pdf(scan_id, url, screenshot_path, scores, legal_text, company, timestamp):
    ...
    def footer():
        ...
        c.drawString(15*mm, 15*mm, f"ApexCore Group d.o.o. | Dossier {scan_id.upper()} | Erstellt: {datetime.utcnow().strftime('%d.%m.%Y %H:%M UTC')}")
    ...
    def page_header(title):
        c.setFillColor(navy); c.rect(0, h-25*mm, w, 25*mm, fill=True, stroke=False)
    ...
    y = h-115*mm
    c.setFillColor(dark); c.setFont("Helvetica-Bold", 10); c.drawString(15*mm, y, "ZIELWEBSITE:")
    c.setFont("Helvetica", 9); c.drawString(15*mm, y-8*mm, url[:80])
    y -= 22*mm; c.setFont("Helvetica-Bold", 10); c.drawString(15*mm, y, "PRÜFDATUM:")
    c.setFont("Helvetica", 9); c.drawString(15*mm, y-8*mm, timestamp)
    y -= 22*mm; c.setFont("Helvetica-Bold", 10); c.drawString(15*mm, y, "ZIELFIRMA (Impressum):")
    c.setFont("Helvetica", 9); c.drawString(15*mm, y-8*mm, company.get("name", "Unbekannt")); c.drawString(15*mm, y-16*mm, company.get("address", ""))
    ...
    if ss_path.exists():
        try:
            ...
            c.drawImage(img, 15*mm, 30*mm, width=w-30*mm, height=h-70*mm, preserveAspectRatio=True)
        except Exception as e:
    ...
    c.setFillColor(dark); y = h-45*mm
    c.setFont("Helvetica-Bold", 10); c.drawString(15*mm, y, "Detection Ensemble Scores:"); y -= 8*mm
    c.setFont("Helvetica", 9)
    for name, val in [("Copyleaks (40% Gewicht, 0.03% FP-Rate)", scores["copyleaks"]), ("GPTZero (30% Gewicht, 3.3% FP-Rate)", scores["gptzero"]), ("Winston AI (20% Gewicht, 1.5% FP-Rate)", scores["winston"]), ("Heuristik (10% Gewicht)", scores["heuristic"])]:
        c.drawString(20*mm, y, f"{name}: {val}%"); y -= 6*mm
    y -= 5*mm; c.setFont("Helvetica-Bold", 10)
    c.drawString(15*mm, y, f"Gewichteter Gesamtscore: {scores['confidence']}%"); y -= 6*mm
