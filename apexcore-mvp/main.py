...
async def generate_legal_text(url: str, scores: dict, company: dict) -> str:
    ...
    if not api_key or api_key.startswith("PLACEHOLDER"): return fallback_legal_text(url, scores, company)
...
Professioneller Ton. Kein Disclaimer nötig (internes Kanzlei-Dokument)."""
    async with httpx.AsyncClient(timeout=60.0) as client:
        try:
            resp = await client.post("https://api.anthropic.com/v1/messages", headers={"x-api-key": api_key, "anthropic-version": "2023-06-01", "content-type": "application/json"}, json={"model": "claude-sonnet-4-20250514", "max_tokens": 1000, "messages": [{"role": "user", "content": prompt}]})
            resp.raise_for_status(); return resp.json()["content"][0]["text"]
        except Exception as e: print(f"Claude API error: {e}"); return fallback_legal_text(url, scores, company)
...
def assemble_pdf(scan_id, url, screenshot_path, scores, legal_text, company, timestamp):
    pdf_path = OUTPUT_DIR / f"dossier_{scan_id}.pdf"
    c = canvas.Canvas(str(pdf_path), pagesize=A4); w, h = A4
    navy=colors.HexColor("#1B3A6B"); white=colors.white; dark=colors.HexColor("#1A1A2E"); gray=colors.HexColor("#6B7A99")
    risk = "HOCH" if scores["confidence"] >= 85 else "MITTEL" if scores["confidence"] >= 60 else "NIEDRIG"
    risk_color = colors.red if risk=="HOCH" else colors.orange if risk=="MITTEL" else colors.green
    page_num = [1]
    def footer(): c.setFillColor(gray); c.setFont("Helvetica",7); c.drawString(15*mm,15*mm,f"ApexCore Group d.o.o. | Dossier {scan_id.upper()} | Erstellt: {datetime.utcnow().strftime('%d.%m.%Y %H:%M UTC')}"); c.drawRightString(w-15*mm,15*mm,f"Seite {page_num[0]}")
    def next_page(): footer(); c.showPage(); page_num[0]+=1
    def page_header(title): c.setFillColor(navy); c.rect(0,h-25*mm,w,25*mm,fill=True,stroke=False); c.setFillColor(white); c.setFont("Helvetica-Bold",14); c.drawString(15*mm,h-16*mm,title)
    c.setFillColor(navy); c.rect(0,h-100*mm,w,100*mm,fill=True,stroke=False)
    c.setFillColor(white); c.setFont("Helvetica-Bold",22); c.drawString(15*mm,h-35*mm,"RECHTLICHES DOSSIER")
    c.setFont("Helvetica",11); c.drawString(15*mm,h-48*mm,"EU AI Act Art. 50 — Enforcement"); c.drawString(15*mm,h-58*mm,"ApexCore Group d.o.o. Beograd"); c.drawString(15*mm,h-68*mm,f"Dossier-ID: {scan_id.upper()}")
    y=h-115*mm; c.setFillColor(dark); c.setFont("Helvetica-Bold",10); c.drawString(15*mm,y,"ZIELWEBSITE:"); c.setFont("Helvetica",9); c.drawString(15*mm,y-8*mm,url[:80])
    y-=22*mm; c.setFont("Helvetica-Bold",10); c.drawString(15*mm,y,"PRÜFDATUM:"); c.setFont("Helvetica",9); c.drawString(15*mm,y-8*mm,timestamp)
    y-=22*mm; c.setFont("Helvetica-Bold",10); c.drawString(15*mm,y,"ZIELFIRMA (Impressum):"); c.setFont("Helvetica",9); c.drawString(15*mm,y-8*mm,company.get("name","Unbekannt")); c.drawString(15*mm,y-16*mm,company.get("address",""))
    y-=34*mm; c.setFillColor(dark); c.setFont("Helvetica-Bold",14); c.drawString(15*mm,y,f"DETECTION SCORE: {scores['confidence']}%")
    y-=12*mm; c.setFillColor(risk_color); c.setFont("Helvetica-Bold",12); c.drawString(15*mm,y,f"RISIKO: {risk}")
    next_page()
    page_header("RECHTLICHE ANALYSE"); c.setFillColor(dark); c.setFont("Helvetica",9)
    y=h-40*mm; line_h=5*mm; max_w=w-30*mm
    for para in legal_text.split("\n\n"):
        if not para.strip(): continue
        lines=[]; line=""
        for word in para.split():
            test=(line+" "+word).strip()
            if c.stringWidth(test,"Helvetica",9)<=max_w: line=test
            else:
                if line: lines.append(line)
                line=word
        if line: lines.append(line)
        for ln in lines:
            if y<25*mm: next_page(); page_header(""); c.setFillColor(dark); c.setFont("Helvetica",9); y=h-35*mm
            c.drawString(15*mm,y,ln); y-=line_h
        y-=line_h
    next_page()
    page_header("BEWEISMATERIAL — Screenshot")
    ss_path=Path(str(screenshot_path))
    if ss_path.exists():
        try: c.drawImage(ImageReader(str(ss_path)),15*mm,30*mm,width=w-30*mm,height=h-70*mm,preserveAspectRatio=True)
        except Exception as e: c.setFillColor(dark); c.setFont("Helvetica",9); c.drawString(15*mm,h-50*mm,f"Screenshot nicht einbettbar: {e}")
    sha=hashlib.sha256(ss_path.read_bytes()).hexdigest()[:32] if ss_path.exists() else "N/A"
    c.setFillColor(gray); c.setFont("Helvetica",7); c.drawString(15*mm,22*mm,f"SHA-256: {sha}...")
    next_page()
    page_header("DETECTION DETAILS + CHAIN OF CUSTODY"); c.setFillColor(dark); y=h-45*mm
    c.setFont("Helvetica-Bold",10); c.drawString(15*mm,y,"Detection Ensemble Scores:"); y-=8*mm
    c.setFont("Helvetica",9)
    for name,val in [("Copyleaks (40% Gewicht, 0.03% FP-Rate)",scores["copyleaks"]),("GPTZero (30% Gewicht, 3.3% FP-Rate)",scores["gptzero"]),("Winston AI (20% Gewicht, 1.5% FP-Rate)",scores["winston"]),("Heuristik (10% Gewicht)",scores["heuristic"])]: c.drawString(20*mm,y,f"{name}: {val}%"); y-=6*mm
    y-=5*mm; c.setFont("Helvetica-Bold",10); c.drawString(15*mm,y,f"Gewichteter Gesamtscore: {scores['confidence']}%"); y-=6*mm; c.drawString(15*mm,y,f"Entscheidung: {scores['decision']}")
    y-=15*mm; c.drawString(15*mm,y,"Chain of Custody:"); y-=8*mm
    now_str=datetime.utcnow().isoformat()+"Z"; c.setFont("Helvetica",8)
    for entry in [f"[{timestamp}] Scan initiiert — ApexCore System",f"[{now_str}] Playwright-Browser gestartet",f"[{now_str}] Screenshot erstellt (1920x1080)",f"[{now_str}] HTML archiviert",f"[{now_str}] SHA-256 Hash berechnet",f"[{now_str}] Detection Ensemble durchgeführt",f"[{now_str}] Rechtliche Analyse generiert",f"[{now_str}] PDF-Dossier erstellt"]: c.drawString(20*mm,y,entry); y-=5*mm
    footer(); c.save(); return pdf_path
