...
async def run_pipeline(scan_id: str, url: str):
    try:
        ...
        SCANS[scan_id].update({
            "status": "completed", "completed": datetime.utcnow().isoformat(),
            "confidence": scores["confidence"], "decision": scores["decision"],
            "pdf_path": str(pdf_path), "company": company, "scores": scores,
        })
...
async def forensic_scrape(url: str, scan_id: str):
    ...
    async with async_playwright() as p:
        ...
        context = await browser.new_context(viewport={"width": 1920, "height": 1080}, user_agent="ApexCore-Forensic-Bot/2.0 (+https://apexcore.group)")
...
async def _copyleaks_bearer() -> Optional[str]:
    ...
    async with httpx.AsyncClient(timeout=20) as client:
        try:
            r = await client.post("https://id.copyleaks.com/v3/account/login/api", json={"email": COPYLEAKS_EMAIL, "key": COPYLEAKS_KEY})
            r.raise_for_status()
...
async def copyleaks_score(text: str) -> Optional[float]:
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        try:
            r = await client.post(
                f"https://api.copyleaks.com/v2/writer-detector/{str(uuid.uuid4())[:20]}/check",
                headers={"Authorization": f"Bearer {token}"},
                json={"text": text[:25000], "sandbox": False},
            )
            r.raise_for_status()
            ai_prob = r.json().get("summary", {}).get("ai")
            return round(float(ai_prob) * 100, 1) if ai_prob is not None else None
        except Exception as e:
...
async def gptzero_score(text: str) -> Optional[float]:
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        try:
            r = await client.post("https://api.gptzero.me/v2/predict/text", headers={"x-api-key": GPTZERO_KEY, "Content-Type": "application/json"}, json={"document": text[:10000]})
            r.raise_for_status()
            return round(float(r.json()["data"]["doc_completely_generated_prob"]) * 100, 1)
        except Exception as e:
...
async def detect(html: str, page_text: str, url: str) -> dict:
    ...
    if components:
        ...
        components.append(("heuristic", heuristic, max(0.20, 1.0 - real_weight)))
    ...
    return {"confidence": round(confidence, 1), "decision": decision, "copyleaks": cl_score, "gptzero": gz_score, "heuristic": round(heuristic, 1), "real_apis": real_apis}


def heuristic_score(html: str) -> float:
    score = 0.0
    lower = html.lower()
    for marker in ["gpt", "chatgpt", "openai", "ki-generiert", "ai-generated", "powered by ai", "written by ai", "created with ai", "claude", "gemini"]:
        if marker in lower:
            score += 12
    word_count = max(len(html.split()), 1)
    tw_count = sum(lower.count(w) for w in ["however", "moreover", "therefore", "furthermore", "additionally", "nevertheless", "consequently", "subsequently", "in conclusion", "it is worth noting", "it should be noted"])
    if tw_count / word_count > 0.008:
        score += 25
    if not any(kw in lower for kw in ["ki-generiert", "ai-generated", "künstliche intelligenz", "erstellt mit ki", "generated by ai", "ki-unterstützt"]):
        score += 20
...
Detection-Methoden: {' | '.join(detection_detail)}
...
def assemble_pdf(scan_id: str, url: str, screenshot_path, scores: dict, legal_text: str, company: dict, timestamp: str) -> Path:
    ...
    w, h = A4
    NAVY = colors.HexColor("#1B3A6B"); WHITE = colors.white; DARK = colors.HexColor("#1A1A2E")
    GRAY = colors.HexColor("#6B7A99"); LGRAY = colors.HexColor("#F3F5F8")
    RED = colors.HexColor("#DC2626"); AMBER = colors.HexColor("#D97706"); GREEN = colors.HexColor("#16A34A")
    ...
    def page_header(title: str, sub: str = ""):
        c.setFillColor(NAVY); c.rect(0, h - 23*mm, w, 23*mm, fill=True, stroke=False)
        c.setFillColor(WHITE); c.setFont("Helvetica-Bold", 12); c.drawString(15*mm, h - 14*mm, title)
        if sub: c.setFont("Helvetica", 8); c.drawString(15*mm, h - 20*mm, sub)

    c.setFillColor(NAVY); c.rect(0, h - 88*mm, w, 88*mm, fill=True, stroke=False)
    c.setFillColor(WHITE); c.setFont("Helvetica-Bold", 24); c.drawString(15*mm, h - 30*mm, "RECHTLICHES DOSSIER")
    c.setFont("Helvetica-Bold", 11); c.drawString(15*mm, h - 42*mm, "EU AI Act Art. 50 — Transparenzpflicht-Analyse")
    c.setFont("Helvetica", 9); c.drawString(15*mm, h - 52*mm, "ApexCore Group d.o.o. Beograd")
    ...
    c.setFont("Helvetica-Bold", 8); c.drawString(15*mm, h - 78*mm, "VERTRAULICH — NUR FÜR KANZLEIINTERNE NUTZUNG")
    y0 = h - 100*mm
    c.setFillColor(LGRAY); c.rect(15*mm, y0 - 55*mm, w - 30*mm, 53*mm, fill=True, stroke=False)
    c.setFillColor(DARK); c.setFont("Helvetica-Bold", 9); c.drawString(19*mm, y0 - 7*mm, "ZIELWEBSITE:")
    c.setFont("Helvetica", 9); c.drawString(19*mm, y0 - 14*mm, url[:85])
    c.setFont("Helvetica-Bold", 9); c.drawString(19*mm, y0 - 22*mm, "ZIELFIRMA:")
    c.setFont("Helvetica", 9); c.drawString(19*mm, y0 - 29*mm, company.get("name", "Unbekannt"))
    if company.get("address", "Nicht gefunden") != "Nicht gefunden":
        c.drawString(19*mm, y0 - 35*mm, company["address"])
    c.setFont("Helvetica-Bold", 13); c.setFillColor(DARK); c.drawString(19*mm, y0 - 46*mm, f"KI-Detection Score: {scores['confidence']}%")
    c.setFillColor(risk_color); c.roundRect(w - 67*mm, y0 - 50*mm, 50*mm, 13*mm, 3*mm, fill=True, stroke=False)
    c.setFillColor(WHITE); c.setFont("Helvetica-Bold", 10); c.drawCentredString(w - 42*mm, y0 - 44*mm, f"RISIKO: {risk}")
    y1 = y0 - 65*mm; c.setFillColor(DARK); c.setFont("Helvetica-Bold", 9)
    c.drawString(15*mm, y1, "PRÜFDATUM:"); c.setFont("Helvetica", 9); c.drawString(15*mm, y1 - 6*mm, timestamp)
    c.setFont("Helvetica-Bold", 9); c.drawString(15*mm, y1 - 14*mm, "DETECTION-MODUS:")
    c.setFillColor(GREEN if scores.get("real_apis") else AMBER); c.setFont("Helvetica", 9)
    c.drawString(15*mm, y1 - 20*mm, "✓ Echte externe APIs" if scores.get("real_apis") else "⚠ Pilot: Heuristik")
    next_page()

    page_header("RECHTLICHE ANALYSE", f"Dossier {scan_id.upper()} | {url[:65]}")
    c.setFillColor(DARK); c.setFont("Helvetica", 9)
    y = h - 32*mm; lh = 5.2*mm; mw = w - 30*mm
    for para in legal_text.split("\n\n"):
        para = para.strip()
        if not para:
            continue
        is_heading = para.isupper() or re.match(r'^\d+\.\s+[A-ZÄÖÜ]', para)
        c.setFont("Helvetica-Bold" if is_heading else "Helvetica", 10 if is_heading else 9)
        for ln_raw in para.split("\n"):
            words = ln_raw.split(); line = ""
            for word in words:
                test = (line + " " + word).strip()
                if c.stringWidth(test, "Helvetica-Bold" if is_heading else "Helvetica", 10 if is_heading else 9) <= mw:
                    ...
                    if y < 22*mm:
                        next_page(); page_header("RECHTLICHE ANALYSE (Forts.)")
                        c.setFillColor(DARK); y = h - 32*mm
                    c.drawString(15*mm, y, line); y -= lh; line = word
            if line:
                if y < 22*mm:
                    next_page(); page_header("RECHTLICHE ANALYSE (Forts.)")
                    c.setFillColor(DARK); y = h - 32*mm
                c.drawString(15*mm, y, line); y -= lh
            c.setFont("Helvetica", 9); is_heading = False
        y -= lh * 0.4
    next_page()
    ...
    if ss_path.exists():
        try:
            c.drawImage(ImageReader(str(ss_path)), 15*mm, 26*mm, width=w - 30*mm, height=h - 58*mm, preserveAspectRatio=True)
        except Exception as e:
            c.setFillColor(DARK); c.setFont("Helvetica", 9); c.drawString(15*mm, h - 50*mm, f"Screenshot nicht einbettbar: {e}")
    sha = hashlib.sha256(ss_path.read_bytes()).hexdigest() if ss_path.exists() else "N/A"
    c.setFillColor(GRAY); c.setFont("Helvetica", 7); c.drawString(15*mm, 19*mm, f"SHA-256: {sha}")
    next_page()

    page_header("DETECTION ENSEMBLE + BEWEISKETTE")
    c.setFillColor(DARK); y = h - 36*mm
    c.setFont("Helvetica-Bold", 10); c.drawString(15*mm, y, "Detection Ensemble Ergebnisse:"); y -= 8*mm
    ...
    for name, val, weight, note in [
        ...
        c.drawString(18*mm, y, f"{name}  [{weight}]:  {val}%  |  {note}" if val is not None else f"{name}:  nicht konfiguriert — Gewicht auf Heuristik umgelegt")
        y -= 6*mm
    y -= 4*mm; c.setFont("Helvetica-Bold", 10)
    c.drawString(15*mm, y, f"Gewichteter Gesamtscore: {scores['confidence']}%"); y -= 7*mm
    c.drawString(15*mm, y, f"Entscheidung: {scores['decision']}"); y -= 7*mm
    c.setFont("Helvetica", 8)
    if scores.get("real_apis"):
        c.setFillColor(GREEN); c.drawString(15*mm, y, "Echte externe APIs verwendet — forensisch verwertbar")
    else:
        c.setFillColor(AMBER); c.drawString(15*mm, y, "HINWEIS: Pilot-Modus — heuristische Analyse (COPYLEAKS_API_KEY / GPTZERO_API_KEY fehlen)")
    c.setFillColor(DARK); y -= 16*mm
    c.setFont("Helvetica-Bold", 10); c.drawString(15*mm, y, "Chain of Custody (Beweiskette):"); y -= 8*mm
    now_str = datetime.utcnow().isoformat() + "Z"; c.setFont("Helvetica", 8)
    for entry in [
        ...
        c.drawString(18*mm, y, entry); y -= 5*mm
    next_page()

    page_header("DATENSCHUTZ & RECHTLICHE HINWEISE", "DSGVO-Compliance | Art. 13 DSGVO")
    c.setFillColor(DARK); y = h - 34*mm
    c.setFont("Helvetica-Bold", 10); c.drawString(15*mm, y, "DATENSCHUTZHINWEIS (Art. 13 DSGVO)"); y -= 10*mm
    label_w = 43*mm; val_x = 15*mm + label_w + 2*mm; val_w = w - 15*mm - label_w - 17*mm
    for label, text in [
        ...
        if y < 30*mm:
            next_page(); page_header("DATENSCHUTZ (Forts.)")
            c.setFillColor(DARK); c.setFont("Helvetica", 8.5); y = h - 34*mm
        c.setFont("Helvetica-Bold", 8.5); c.drawString(15*mm, y, f"{label}:")
        c.setFont("Helvetica", 8.5); words = text.split(); line = ""; first_line = True
        for word in words:
            test = (line + " " + word).strip()
            if c.stringWidth(test, "Helvetica", 8.5) <= val_w:
                line = test
            else:
                c.drawString(val_x, y, line); y -= 5*mm
                if first_line: first_line = False
                line = word
        if line:
            c.drawString(val_x, y, line)
        y -= 8*mm
    y -= 4*mm; c.setFont("Helvetica-Bold", 9); c.drawString(15*mm, y, "HAFTUNGSAUSSCHLUSS:"); y -= 6*mm
    c.setFont("Helvetica", 8)
