...
# NOTE: /dossier/latest before /dossier/{scan_id} — FastAPI route order matters
...
async def run_pipeline(scan_id: str, url: str):
    try:
        ...
        pdf_path = assemble_pdf(
            scan_id, url, screenshot_path, scores, legal_text, company, timestamp
        )
...
async def _copyleaks_bearer() -> Optional[str]:
    ...
    if _cl_token and (time.time() - _cl_token_ts) < 160_000:  # 44 h buffer
        return _cl_token
...
async def detect(html: str, page_text: str, url: str) -> dict:
    ...
    cl_score, gz_score = await asyncio.gather(
        copyleaks_score(text),
        gptzero_score(text),
    )
...
def extract_impressum(html: str) -> dict:
    ...
    for pat in [
        ...
        r'(?:GmbH|AG|UG|e\.K\.|KG|OHG|GbR|Ltd\.|Inc\.|SE|GmbH\s*&\s*Co\.?\s*KG))',
    ...
    phone = re.search(
        r'(?:Tel|Telefon|Phone)[.:\s]*(\+?[\d\s\-/()]{8,20})', html, re.IGNORECASE
    )
    if phone:
...
Art. 50 Abs. 4 EU AI Act verpflichtet Betreiber von KI-Systemen zur Kennzeichnung KI-generierter oder manipulierter Text-, Bild-, Audio- und Videoinhalte, sofern diese geeignet sind, Personen zu täuschen. Die geprüfte Website weist keine den Anforderungen entsprechenden Transparenzhinweise auf KI-Einsatz auf. Dies begründet prima facie einen Verstoß gegen Art. 50 Abs. 4 i.V.m. Art. 99 Abs. 4 EU AI Act (Bußgeld bis EUR 15 Mio. oder 3 % des Jahresumsatzes).

BEWEISLAGE

Forensische Beweissicherung: Screenshot (1920×1080), HTML-Vollarchiv, SHA-256-Hash — gerichtsverwertbar gesichert. Detection-Score: {score}% Konfidenz (Entscheidung: {decision}). Beweismittel als PDF-Anlage beigefügt.
...
def _wrap_text(c_obj, text: str, x: float, y: float, max_w: float,
               font: str, size: float, line_h: float) -> float:
    words = text.split()
    line = ""
    for word in words:
...
def assemble_pdf(
    scan_id: str, url: str, screenshot_path, scores: dict,
    legal_text: str, company: dict, timestamp: str,
) -> Path:
    ...
    risk = (
        "HOCH"   if scores["confidence"] >= 85 else
        "MITTEL" if scores["confidence"] >= 60 else
        "NIEDRIG"
    )
    ...
    def footer():
        ...
        c.drawString(
            15*mm, 11*mm,
            f"ApexCore Group d.o.o. Beograd  |  Dossier {scan_id.upper()}  |  "
            f"{datetime.utcnow().strftime('%d.%m.%Y %H:%M UTC')}  |  VERTRAULICH",
        )
    ...
    c.drawString(15*mm, h - 60*mm,
        f"Referenz: AC-{scan_id.upper()}-{datetime.utcnow().strftime('%Y%m%d')}")
    c.drawString(15*mm, h - 68*mm,
    ...
    if ss_path.exists():
        try:
            ...
            c.drawImage(img, 15*mm, 26*mm, width=w - 30*mm, height=h - 58*mm,
    ...
    # ── PAGE 4: DETECTION DETAILS + CHAIN OF CUSTODY ──────────────────────────
    ...
    for name, val, weight, note in detection_rows:
        if val is not None:
            line = f"{name}  [{weight}]:  {val}%  |  {note}"
        else:
            line = f"{name}:  nicht konfiguriert — Gewicht auf Heuristik umgelegt"
    ...
    if scores.get("real_apis"):
        ...
        c.drawString(15*mm, y,
            "HINWEIS: Pilot-Modus — heuristische Analyse (COPYLEAKS_API_KEY / GPTZERO_API_KEY fehlen)")
    c.setFillColor(DARK)
    ...
    # ── PAGE 5: DSGVO + RECHTLICHE HINWEISE ───────────────────────────────────
    ...
    dsgvo_rows = [
        ("Verantwortlicher",
         "ApexCore Group d.o.o. Beograd, Serbien | legal@apexcore.group"),
        ("Rechtsgrundlage",
         "Art. 6 Abs. 1 lit. f DSGVO — Berechtigte Interessen "
         "der beauftragenden Kanzlei an EU AI Act Compliance-Prüfung"),
        ("Verarbeitungszweck",
         "Forensische Analyse öffentlich zugänglicher Websites "
         "auf Verstöße gegen Art. 50 EU AI Act (VO (EU) 2024/1689)"),
        ("Datenquellen",
         "Ausschließlich öffentlich zugängliche Webseiten; keine personenbezogenen "
         "Daten werden gezielt erhoben oder verarbeitet"),
        ("Speicherdauer",
         "90 Tage; danach automatische Löschung aller Dossier-Daten (Screenshots, PDFs)"),
        ("Externe Dienste",
         "Copyleaks Ltd. (EU-Server, auftragsverarbeitet); "
         "GPTZero Inc. (USA, SCC); Anthropic PBC (USA, SCC)"),
        ("Betroffenenrechte",
         "Auskunft, Berichtigung, Löschung, Widerspruch — Anfragen: legal@apexcore.group"),
        ("Aufsichtsbehörde",
         "Bundesbeauftragte für den Datenschutz und die Informationsfreiheit (BfDI), Bonn"),
    ]
    ...
    c.setFont("Helvetica", 8)
    disclaimer = (
        "Dieses Dossier dient als Entscheidungsgrundlage für die beauftragende Kanzlei. "
        "Die Detection-Scores stellen keine rechtskräftige Feststellung dar. "
        "Eine abschließende juristische Bewertung obliegt dem zuständigen Rechtsanwalt. "
        "ApexCore Group haftet nicht für Entscheidungen, die auf Basis dieses Dokuments getroffen werden."
    )
    y = _wrap_text(c, disclaimer, 15*mm, y, w - 30*mm, "Helvetica", 8, 4.5*mm)
